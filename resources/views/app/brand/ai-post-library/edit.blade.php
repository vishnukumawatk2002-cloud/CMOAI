...
@section('title', 'Edit post — CMO AI')
@section('pageTitle', 'Edit post')

@section('content')
<div class="card" style="max-width:720px">
    <div class="card-title">Edit {{ ucfirst($contentItem->content_type ?? 'post') }}</div>
    <form method="POST" action="{{ route('app.brand.ai-post-library.update', $contentItem) }}">
        @csrf
        @method('PUT')
        <div class="field-row">
            <div class="field">
                ...
                <select id="platform" name="platform">
                    @foreach(['instagram','x','linkedin','facebook','youtube'] as $p)
        ...
        <div style="display:flex;gap:10px;margin-top:8px">
            ...
            <a href="{{ route('app.brand.ai-post-library') }}" class="btn btn-ghost">Cancel</a>
        </div>
    </form>
</div>
@endsection
